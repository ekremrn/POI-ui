...
try:
    ...
    predicted_data = list(analyser_collection.find(query))
    actual_data = list(charts_collection.find(query))
    ...
    if not predicted_data or not actual_data:
        ...
        for ticker in tickers:
            ...
            if ticker_predicted and ticker_actual:
                ...
                next_predictions = [p for p in ticker_predicted if p["time"] > current_time][:4]
                
                # Calculate position signal
                if len(next_predictions) >= 4:
                    all_higher = all(p["price"] > current_price for p in next_predictions)
                    all_lower = all(p["price"] < current_price for p in next_predictions)
                    avg_diff = sum((p["price"] - current_price) / current_price * 100 for p in next_predictions) / len(next_predictions)
                
                next_predictions_avg = sum(p["price"] for p in next_predictions) / len(next_predictions)
                price_diff = ((next_predictions_avg - current_price) / current_price) * 100
